...
top_model_weights_path = '1505.h5'
...
def predict():
    ...
    if (np.count_nonzero(predict_proba)) < 4:
        ...
        label = inv_map[inID]
        print("\nImage ID: {}, Label: {}".format(inID, label))
        var = str("Image ID: {}, Label: {}".format(inID, label))
    else: 
       # print("\nThis person isnt in this database")
        var = 'This person isnt in this database'
    return var
...
def test():
   ...
   #                 input_shape = (img_width, img_height,3))
    ...
    generator = datagen_top.flow_from_directory(
        test_data_dir,
        target_size=(img_width, img_height),
        batch_size=batch_size,
        class_mode='categorical',
   ...
   #predict_size_train = int(math.ceil(nb_train_samples / batch_size))
    ...
    #np.save('bottleneck_features_test_xxx.npy',
           # bottleneck_features_train)

    #test_data = np.load('bottleneck_features_test_xxx.npy')


    model = Sequential()
    model.add(Flatten(input_shape = test_data.shape[1:]))
    model.add(Dropout(0.5))
    model.add(Dense(2048, kernel_initializer = 'glorot_uniform'))
    model.add(PReLU())
    model.add(Dropout(0.5))
    ...
    #model = load_model('model_train_xxx.h5')
    test_labels = np.load('class_indices.npy')
  ...
  #  print("[INFO] accuracy: {:.2f}%".format(eval_accuracy * 100))  
   # print("[INFO] Loss: {}".format(eval_loss)) 
    ...
    PREDICTED_CLASSES = model.predict_classes(generator, batch_size=batch_size,verbose=1)
    print(PREDICTED_CLASSES)
    temp=sum(test_labels == PREDICTED_CLASSES)
    print(temp)
    print(temp/np.size(test_labels))
